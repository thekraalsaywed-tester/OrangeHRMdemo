beforeEach(' visit login   ' ,function(){
...
// assertion
        cy.get('.oxd-table-body').should('contain', 'Arwaa').then(() => cy.log('Job title verified'));
        cy.wait(4000);
...
cy.get(':nth-child(2) > .oxd-input').type('Software Tester') //type job title
cy.wait(4000)
...
// assertion
        cy.get('.oxd-table-body').should('contain', 'Software Tester').then(() => cy.log('Job title verified'));
        cy.wait(4000);
        // First find the exact row containing your job title and scrollview
        cy.contains('.oxd-table-row', 'Software Tester')
            .scrollIntoView({ duration: 300 })
            .should('exist')
            .within(() => {
                // checkbox click
